...
def _normalize_strategy_mode(raw: str | None) -> str:
    ...
    if s in ("kirito", "kirito_early4", "early4", "early_4strike", "prearmed_4strike"):
        return "kirito_early4"
...
class BotConfig:
    ...
    kirito_state_path: str = "exports/kirito_state.json"
    ...
    def from_env(cls) -> "BotConfig":
        ...
        cfg = cls(
            ...
            kirito_state_path=(
                os.getenv("KIRITO_STATE_PATH", "exports/kirito_state.json").strip()
                or "exports/kirito_state.json"
            ),
        )
        ...
        if cfg.strategy_mode == "kirito_early4":
            if cfg.signature_type not in (0, 1, 2, 3):
                raise BotConfigError(
                    "POLY_SIGNATURE_TYPE must be 0-3 "
                    f"(got {cfg.signature_type})."
            ...
            if cfg.kirito_symbol != "BTC":
                raise BotConfigError(
                    "KIRITO_SYMBOL must be BTC for this backtested strategy."
                )
        return cfg
